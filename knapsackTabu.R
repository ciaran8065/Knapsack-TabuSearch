                      ...
                      verbose = FALSE)
{
  ...
  if (is.null(config)) { #if no config create a default.
    ...
    config<-allConfigs[which.max(values[,1]),]
  ...
  for (j in 1:repeatAll) { #ALGORITHM START POINT
    ...
    preliminarySearch <- function() {
      ...
      for (i in 2:iters) { #see comment above to explanation of 2:iters
        ...
        #print(configTemp)
        neighboursEUtility[randomNeighbours,] <- t(apply(configTemp, 1, eval))#"neighboursEUtility" will hold the value of the objective function calculated at each configuration
        #print(neighboursEUtility)
